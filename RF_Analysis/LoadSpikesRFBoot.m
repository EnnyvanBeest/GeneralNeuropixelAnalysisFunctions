    ...
    for didx = 1:length(Dates4Mouse)
        ...
        for sesidx = 1:length(RFsess)
            ...
            for probeid = 1:length(subksdirs)
                ...
                if ~Redo && exist(fullfile(SaveRFDir,MiceOpt{midx},thisdate,thisses,thisprobe,'RFData.mat')) && datetime(tmpfile.date)>= FromDate
...
%                     movefile(fullfile(SaveRFDir,MiceOpt{midx},thisdate,thisses,thisprobe),fullfile(SaveRFDir,MiceOpt{midx},thisdate,thisses,thisprobe,'old'))
                ...
                try
                    ...
                    disp(ME)
                    PipelineParams = PrepareClusInfo(subsesopt,PipelineParams)
                    [clusinfo, sp, Params] = LoadPreparedClusInfo(subsesopt,PipelineParams);
                ...
                SpikeCountPerTP = arrayfun(@(X) histcounts(spikeTimesCorrected(clu_sp'==X),newtimebins),cluster_idUsed(Good_IDx),'UniformOutput',0);
...
%                     clusid
%                     SpikesThisCluster = spikeTimesCorrected(spikeCluster'== cluster_idUsed(Good_IDx(clusid)));
...
%                 end
                ...
                if drawthis
                    for unitid=sigunits
                        figure('name',[areashere{unitid} 'RF, Cell' num2str(cluster_idUsed(Good_IDx(InclUnits(unitid)))) ', RFfit p = ' num2str(pvalsON(unitid))])
                        ...
                        colormap gray
                        drawnow
                        saveas(gcf,fullfile(SaveRFDir,MiceOpt{midx},thisdate,thisses,thisprobe,['Unit' num2str(cluster_idUsed(Good_IDx(InclUnits(unitid)))) '_' areashere{unitid} '-RF.fig']))
                        saveas(gcf,fullfile(SaveRFDir,MiceOpt{midx},thisdate,thisses,thisprobe,['Unit' num2str(cluster_idUsed(Good_IDx(InclUnits(unitid)))) '_' areashere{unitid} '-RF.bmp']))
                    end
                end
                ...
                if size(UniqueID(Good_IDx(InclUnits)),1) ~= height(RFInfo)
                    UniqueID = UniqueID';
                end
                RFInfo.UniqueID = UniqueID(Good_IDx(InclUnits));
